...
public class Database {
	 ...
	 * 
	 * @param reset
	 *            true to create a new empty database
	 */
	...
	public void registerProfessor(String email, String name, String password) {
		int id = generateId("User");
		Professor professor = new Professor(email, name, password, id); // Constructor
	...
	private ArrayList<SubjectProfessor> loadSubjectProfessorsFromFile() throws Exception {
		...
		ArrayList<SubjectProfessor> subjectProfessors_list = (ArrayList<SubjectProfessor>) subjectProfessors; // type
																												// casting
		ois.close();
	...
	public int generateId(String klass) {
		...
		case "User":
			...
			break;
		case "Subject":
			id = getSubjects().size() + 1;
			break;
		case "SubjectProfessor":
			id = getSubjectProfessors().size() + 1;
			break;
		}
		return id;
	}
	...
	public void createRating(int studentId, int professorId, int subjectId, int score) {
		...
		ratings.add(rating); // add the newly created rating to the ratings
								// ArrayList
	}
